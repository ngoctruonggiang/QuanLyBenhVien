...
const PATIENT_INVOICE_ID =
  process.env.BILLING_PATIENT_INVOICE_ID ||
  process.env.PATIENT_INVOICE_ID ||
  "inv-1";
...
test.describe("Billing - Admin", () => {
  ...
  test("Bộ lọc trạng thái hiển thị đủ tùy chọn", async ({ page }) => {
    ...
    await expect(
      page.getByRole("option", { name: /Partially Paid/i }),
    ).toBeVisible();
  ...
  test.describe("Ghi nhận thanh toán", () => {
    test("Form thanh toán có Amount/Method/Notes và submit", async ({
      page,
    }) => {
...
test.describe("Billing - Patient", () => {
  test("Patient xem danh sách hóa đơn của mình", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Hóa đơn của tôi/i }),
    ).toBeVisible();
  ...
  test("Patient xem chi tiết hóa đơn của mình", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Hóa đơn #/i }),
    ).toBeVisible();
    await expect(
      page.getByText(/Thanh toán|Tải biên lai|Quá hạn/i),
    ).toBeVisible();
  });
  ...
  test("Patient thanh toán", async ({ page }) => {
    ...
    await expect(
      page.getByRole("heading", { name: /Thanh toán hóa đơn/i }),
    ).toBeVisible();
    await expect(page.getByLabel(/Payment Amount/i)).toBeVisible();
    await expect(page.getByLabel(/Payment Method/i)).toBeVisible();
    ...
    await page
      .getByRole("button", { name: /Record Payment|Thanh toán/i })
      .click();
    await expect(
      page.getByText(/Amount must be greater than 0/i),
    ).toBeVisible();
